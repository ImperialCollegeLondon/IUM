...
-/
#check biUnion_filter_eq_of_maps_to

-- could you have guessed the name ?
...
-/
#check disjiUnion_filter_eq_of_maps_to

#check sum_disjiUnion
...
theorem harmonic_lb : ∀ n : ℕ, (n / 2 : ℚ) ≤ ∑ k in Icc 1 ((2 : ℕ) ^ n), (1 / k : ℚ) :=
  ...
  · rw [Nat.succ_eq_add_one]
    ...
    obtain ⟨split_disj, split_union⟩ :=
      Nat.Icc_split 1 (2 ^ n) (2 ^ (n + 1)) (by apply pow_le_pow; norm_num; apply Nat.le_succ)
        le_add_self
    rw [split_union]
