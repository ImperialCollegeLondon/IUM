...
theorem takeRight_suffix (n : ℕ) (l : List α) : l.takeRight n <:+ l :=
  drop_suffix _ _

theorem length_takeRight (n : ℕ) (l : List α) : (l.takeRight n).length = min n l.length :=
  (length_drop _ _).trans <| by rw [tsub_tsub_eq_min, min_comm]

@[simp]
theorem take_reverse (n : ℕ) (l : List α) : l.reverse.take n = (l.takeRight n).reverse := by
  rw [rtake_eq_reverse_take_reverse, reverse_reverse]

@[simp]
theorem takeRight_reverse (n : ℕ) (l : List α) : l.reverse.takeRight n = (l.take n).reverse := by
  rw [rtake_eq_reverse_take_reverse, reverse_reverse]

@[simp]
theorem takeRight_takeRight (n m) (l : List α) :
    (l.takeRight m).takeRight n = l.takeRight (min n m) := by
  rw [rtake_eq_reverse_take_reverse, ← take_reverse, take_take, rtake_eq_reverse_take_reverse]

@[simp]
theorem dropRight_append_takeRight (n : ℕ) (l : List α) : l.dropRight n ++ l.takeRight n = l :=
  take_append_drop _ _

theorem suffix_iff_eq_takeRight : l₁ <:+ l₂ ↔ l₁ = l₂.takeRight (length l₁) :=
  ⟨fun h =>
    append_left_cancel <| (suffix_iff_eq_append.1 h).trans (dropRight_append_takeRight _ _).symm,
    fun e => e.symm ▸ takeRight_suffix _ _⟩
...
theorem exists_suffix_length_eq (hn : n ≤ l.length) : ∃ l', l' <:+ l ∧ l'.length = n :=
  ⟨l.takeRight n, takeRight_suffix _ _, (length_takeRight _ _).trans <| min_eq_left hn⟩
