...
#align_import «2022».relations.pals

namespace Relations2022Pals
...
instance : CoeFun S fun a => X → a.Y :=
  ⟨fun a => a.f⟩

-- Let's define what it means for two elements of S to be pals
def Pals (a b : S) : Prop :=
  let-- Say f : X → Y and g : X → Z are in S
    ⟨Y, f, hf⟩ :=
    a
  let ⟨Z, g, hg⟩ := b
...
theorem pals_iff_relations_equal : ∀ f g : S, Pals f g ↔ ∀ s t : X, f s = f t ↔ g s = g t :=
  by
  ...
  · -- Now the other way: assume that ∀ x₁ and x₂, f x₁ = fx₂ ↔ g x₁ = g x₂
    rintro hrel : ∀ x₁ x₂, f x₁ = f x₂ ↔ g x₁ = g x₂
