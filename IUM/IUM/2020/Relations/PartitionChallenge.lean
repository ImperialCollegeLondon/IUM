...
#align_import «2020».relations.partition_challenge

namespace PartitionChallenge
...
theorem mem_of_mem (hX : X ∈ P.c) (hY : Y ∈ P.c) {a b : α} (haX : a ∈ X) (haY : a ∈ Y)
    (hbX : b ∈ X) : b ∈ Y := by sorry

/-- Every term of type `α` is in one of the blocks for a partition `P`. -/
theorem mem_block (a : α) : ∃ X : Set α, X ∈ P.c ∧ a ∈ X := by sorry
...
/-- x is in cl(x) -/
theorem mem_cl_self (a : α) : a ∈ cl R a := by sorry

theorem cl_sub_cl_of_mem_cl {a b : α} : a ∈ cl R b → cl R a ⊆ cl R b := by sorry

theorem cl_eq_cl_of_mem_cl {a b : α} : a ∈ cl R b → cl R a = cl R b := by sorry
